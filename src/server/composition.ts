...
import {
  ...
  forcedBriefFromLines,
  SOFT_LANDING_COMMANDS,
  countSpokenWords,
  isExecutePhrase,
  soleActiveUpid,
  suggestionSpeech,
  projectorState,
  liveProjectorSuggestion,
  resolveMaxConcurrentProcesses,
  resolveRunCompletionPollMs,
  delay,
  resolveSelfReloadDelayMs,
  MIC_ENDPOINTING_BASE_MS,
  DEFAULT_AUTOBUILD_SETTLE_MS,
  GUIDED_HOLD_TTL_MS,
  STEER_GRACE_MS,
  slugFromSpeech,
  readAutoBuildSettleMs,
  candidateIdFromSuggestionId,
...
import {
  type ProjectorRuntimeSubscriber,
  type MicSession,
  type SteerScope,
  type ProjectorRuntimeEnv,
  type ProjectorRuntime,
  type ImportProjectResult,
  type DeckAnswer,
  type ExecuteProcessResult,
  type SeededProcessView,
  type ProjectorRuntimeOptions,
  type DuplicateSpawnGuardOptions,
...
import { RunEventDriver } from "./run-event-driver";
...
import { selectAudioSink, type AudioSinkMode } from "./audio-device-sink";
import { IdeaBuildRegistry, servePreviewDirectory } from "./idea-builder";
...
import { SuggestionEngine, type PendingQueuedSuggestion } from "../suggest/engine";
...
import { includesPhrase, loadRoutingVocabulary, normalizeSpeech, type RoutingVocabulary } from "../routing/vocabulary";
...
import type { IdeaCandidate, PlanQuestion } from "../detect";
import { StageSequencer, type CanonicalStage } from "../spine/stage-sequencer";
import { type LogEvent, type OutputDecision, type PendingSuggestion } from "../types";
import { demoProjectorSnapshot, emptyProjectorSnapshot, withUnmuted } from "../ui/demo-data";
import type { DialogueTurn, IdeaTrayItem, ProjectorSnapshot, ProjectorSuggestion, ResearchTrayItem, TranscriptLine } from "../ui/types";
...
class LiveProjectorRuntime implements ProjectorRuntime {
  ...
  readonly #selfVersions: SelfVersionManager;
  ...
  constructor(
    ...
    this.#selfVersions = new SelfVersionManager({
      env, sessionId, git: (args) => this.#selfGit(args), gh: (args) => this.#selfGh(args),
      trace: (event) => this.recordExternalTrace(event), exit: (code) => this.#exit(code),
    });
    ...
    this.registry = new ProcessRegistry({
      ...
      runIdNonce,
      allocateUpid: () => `upid-${crypto.randomUUID()}`,
  ...
  async selfBranches(): Promise<{ current: string; branches: Array<{ name: string; subject: string; date: string }> }> {
    return this.#selfVersions.selfBranches();
  }
